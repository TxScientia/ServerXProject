...
| Space admin/member | Character | one Space | `memberships` row |
...
| owner_character_id | GUID → characters | creator; auto-gets an `admin` membership |
| title | string | |
...
Unique `(space_id, character_id)`. Same character can be admin of one Space and member of
another — role lives on this link, not on the character or the space.
...
| title | string, nullable | optional group name |
| created_at | datetime | |
